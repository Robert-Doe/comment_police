...
(() => {
    ...
    const CFG = {
        ...
        // (More intense than your previous palette; still a smooth gradient)
        colors: [
            "#fee0d2",// very light
            "#fcbba1",
            "#fc9272",
        ...
        // IMPORTANT: non-painted nodes must stay visually "plain"
        nonPaintFill: "white",
    ...
    function fillColor(count) {
        // Painted but 0 features → plain white
        if (!count || count <= 0) return CFG.nonPaintFill; // "white"
     ...
     * - label: tag only
     * - if data-paint=true: thick border + heat fill
     * - if NOT painted: do not apply any gray fill at all
     ********************************************************************/
    function buildDotWholePage({ rootEl = document.body, title = CFG.dotTitle } = {}) {
        ...
        while (stack.length) {
            ...
            if (!nodes.has(id)) {
                ...
                // Default for NOT painted: plain node (no gray fill)
                let fill = CFG.nonPaintFill; // "white"
    ...
    function run({
        ...
        console.log("✅ Done: core nodes painted; thick border; non-painted nodes left plain; heat colors more intense.");
