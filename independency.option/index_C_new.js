(() => {
    if (!window.__commentFeatureScanPerNode?.results?.length) {
        console.warn("Run your scan first so __commentFeatureScanPerNode.results exists.");
        return;
    }
     ...
     ********************************************************************/
    const byEl = new Map();
    for (const r of __commentFeatureScanPerNode.results) byEl.set(r.el, r);
