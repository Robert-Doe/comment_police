(() => {
    ...
    function detectMetadataNode(el) {
        ...
        let has_author = false;
        {
            ...
            if (containsKeyword(blob, AUTHOR_KEYWORDS)) {
                const t = normalizeText(directTextOf(el));
                if (t.length >= 2 && t.length <= 40) has_author = true;
            }
        }
        ...
        let has_avatar = false;
        {
            ...
            if (containsKeyword(blob, AVATAR_KEYWORDS)) {
                if (el.tagName === "IMG") {
                    const w = el.naturalWidth || el.width || 0;
                    const h = el.naturalHeight || el.height || 0;
                    if (!w || !h || (w <= 250 && h <= 250)) has_avatar = true;
                } else {
                    has_avatar = true;
                }
            }
        ...
        let has_timestamp = false;
        {
            if (el.tagName === "TIME") {
                ...
                for (const attr of el.attributes) {
                    const n = safeStr(attr.name).toLowerCase();
                    if (TIMESTAMP_ATTR_NAMES.includes(n)) { has_timestamp = true; break; }
                }
            }
