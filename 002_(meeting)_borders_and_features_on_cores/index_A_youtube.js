(() => {
    ...
    // IMPORTANT CHANGE:
    // - "prune" mode: only returns false when the subtree is truly invisible (display:none, visibility:hidden, opacity:0)
    ...
    function isVisible(el, mode = "record") {
        ...
        // If these are true, the entire subtree is effectively invisible => safe to prune.
        if (cs.display === "none") return false;
        if (cs.visibility === "hidden") return false;
        if (parseFloat(cs.opacity) === 0) return false;

        // Only require a non-zero rect when deciding whether to RECORD the element.
        // Do NOT use rect=0 to prune traversal (common on YouTube wrappers/containers).
        if (mode === "record") {
    ...
    // IMPORTANT CHANGE:
    // TreeWalker doesn't pierce shadow DOM. This capped walk does (for OPEN shadow roots).
    function cappedTreeWalk(rootEl, whatToShow, maxCount) {
        const out = [];
        if (!rootEl) return out;

        const wantText = whatToShow === NodeFilter.SHOW_TEXT;
        const wantElement = whatToShow === NodeFilter.SHOW_ELEMENT;

        // Start with descendants (to mimic TreeWalker.nextNode() behavior)
        const stack = [];

        function pushChildren(node) {
            if (!node) return;

            // If it's an element, include its light DOM children + open shadow DOM children
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (sr && sr.childNodes && sr.childNodes.length) {
                    for (let i = sr.childNodes.length - 1; i >= 0; i--) stack.push(sr.childNodes[i]);
                }
        ...
        while (stack.length && out.length < maxCount) {
            ...
            if (wantText && node.nodeType === Node.TEXT_NODE) out.push(node);
            else if (wantElement && node.nodeType === Node.ELEMENT_NODE) out.push(node);
    ...
    /********************************************************************
     * Individual Positive Feature Detectors
     ********************************************************************/
    ...
    function detectRelatedKeyword(rootEl) {
        ...
        for (const el of els) {
            ...
            if (matchesRelated(cls) || matchesRelated(id)) { has_related_keyword = true; break; }

            const aria = el.getAttribute?.("aria-label");
            const title = el.getAttribute?.("title");
            if (matchesRelated(aria) || matchesRelated(title)) { has_related_keyword = true; break; }

            const txt = normalizeText(el.textContent || "");
            if (txt && txt.length <= 60 && matchesRelated(txt)) { has_related_keyword = true; break; }
    ...
    while (stack.length && visited < OPT.maxElements) {
        ...
        // IMPORTANT CHANGE:
        // If requireVisible is on, we only PRUNE the subtree when it's truly invisible (display:none etc.).
        if (OPT.requireVisible && !isVisible(el, "prune")) {
            continue;
        }

        // IMPORTANT CHANGE:
        // We only RECORD this node if it's visible in the "record" sense (includes rect check).
        ...
        // IMPORTANT CHANGE:
        // Always traverse children (even if parent has rect=0), and also traverse open shadow roots.
        const children = [];
